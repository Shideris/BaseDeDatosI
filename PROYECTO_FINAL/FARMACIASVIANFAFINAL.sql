...
--CREACION DE UNA TABLA PARA LA DEMOSTRACION DEL CODIGO DE ENCRIPTACION
...
insert into USUARIO(NOMBRE, APELLIDOS, USUARIO, CONTRASENA) values
('FABIVA','RAMOS', 'FABI', PWDENCRYPT('7535F')),
('ANA', 'CALDERON', 'ANA', PWDENCRYPT('ANA1'));

select *
from USUARIO
WHERE USUARIO='FABI' AND PWDCOMPARE('7535F',CONTRASENA)=1;
...
AS
BEGIN
   INSERT INTO USUARIO VALUES (@NOMBRE, @APELLIDO, @USUARIO, ENCRYPTBYPASSPHRASE('PASSWORD', @CONTRASENA))
end
    go;

-- AGREGAR REGISTRO
EXEC REGISTRA 'VICTOR', 'ORTEGA', 'VICTOR', '2828'

-- CONSULTAR TODA LA TABLA

select * from USUARIO;

--DESENCRIPTAR LAS CONTRASEÑAS SOLO POR EM PROCEDIMIENTO

select convert(nvarchar(max), decryptbypassphrase('PASSWORD', CONTRASENA)) from USUARIO;


Select * from PERSONAL where USUARIO= 'ALE' and CONVERT(nvarchar(max), decryptbypassphrase('PASSWORD', CONTRASENA))= 'ALEAD'
